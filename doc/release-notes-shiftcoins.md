Shiftcoins Core version 0.18.1 is now available from:

  <<https://download.shiftcoins.org/shiftcoins-0.18.1/>
...
To receive security and update notifications, please subscribe to:
  ...
  <https://groups.google.com/forum/#!forum/shiftcoins-dev>
...
`/Applications/Shiftcoins-Qt` (on Mac) or `shiftcoinsd`/`shiftcoins-qt` (on
Linux).
...
Shiftcoins Core is supported and extensively tested on operating systems
using the Linux kernel, macOS 10.10+, and Windows 7 and newer. It is not
recommended to use Shiftcoins Core on unsupported systems.

Shiftcoins Core should also work on most other Unix-like systems but is not
as frequently tested on them.

From 0.17.0 onwards, macOS <10.10 is no longer supported. 0.17.0 is
built using Qt 5.9.x, which doesn't support versions of macOS older than
10.10. Additionally, Shiftcoins Core does not yet change appearance when
...
- Four new options are available for configuring the maximum number of
  ...
  documentation](https://github.com/shiftcoins-project/shiftcoins/blob/master/doc/zmq.md#usage)
  for details.
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](mailto:contact@shiftcoins.org) the Shiftcoins Core
...
- A new short [document](https://github.com/shiftcoins-project/shiftcoins/blob/master/doc/JSON-RPC-interface.md)
  about the JSON-RPC interface describes cases where the results of an
  RPC might contain inconsistencies between data sourced from different
  subsystems, such as wallet state and mempool state.  A note is added
  to the [REST interface documentation](https://github.com/shiftcoins-project/shiftcoins/blob/master/doc/REST-interface.md)
  indicating that the same rules apply.

- Further information is added to the [JSON-RPC
  documentation](https://github.com/shiftcoins-project/shiftcoins/blob/master/doc/JSON-RPC-interface.md)
  about how to secure this interface.

- A new [document](https://github.com/shiftcoins-project/shiftcoins/blob/master/doc/shiftcoins-conf.md)
  about the `shiftcoins.conf` file describes how to use it to configure
  Shiftcoins Core.

- A new document introduces Shiftcoins Core's BIP174 [Partially-Signed
  Shiftcoins Transactions
  (PSBT)](https://github.com/shiftcoins-project/shiftcoins/blob/master/doc/psbt.md)
...
- The [output script
  descriptor](https://github.com/shiftcoins-project/shiftcoins/blob/master/doc/descriptors.md)
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent Shiftcoins-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  Shiftcoins Core to libssl vulnerabilities in the past, builders who don't
...
- The minimum required version of Qt (when building the GUI) has been
  ...
  system](https://github.com/shiftcoins-project/shiftcoins/blob/master/depends/README.md)
  provides 5.9.7)
...
- `deriveaddresses` returns one or more addresses corresponding to an
  [output descriptor](https://github.com/shiftcoins-project/shiftcoins/blob/master/doc/descriptors.md).
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when Shiftcoins Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of Shiftcoins Core, subsequently importing
...
- `importmulti` now supports importing of addresses from descriptors. A
  ...
  [here](https://github.com/shiftcoins-project/shiftcoins/blob/master/doc/descriptors.md).
...
- The 'account' API is removed after being deprecated in v0.17.  The
  ...
  v0.17](https://github.com/shiftcoins-project/shiftcoins/blob/master/doc/release-notes/release-notes-0.17.0.md#label-and-account-apis-for-wallet)
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart shiftcoinsd with the `-deprecatedrpc=generate`
...
- A new `shiftcoins-wallet` tool is now distributed alongside Shiftcoins
...
This section describes planned changes to Shiftcoins Core that may affect
other Shiftcoins software and services.

- Since version 0.16.0, Shiftcoins Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with Shiftcoins Core 0.20 (expected about
  a year after 0.18), Shiftcoins Core will default to native segwit
  ...
  to bech32 addresses, and if the Shiftcoins Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in Shiftcoins Core 0.19 (approximately November 2019).
  ...
  set the `addresstype=bech32` configuration option in any Shiftcoins Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, Shiftcoins Core
...
- A sub-project of Shiftcoins Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with Shiftcoins Core.  See their [project
...
- This release changes the Random Number Generator (RNG) used from
  OpenSSL to Shiftcoins Core's own implementation, although entropy
  gathered by Shiftcoins Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves Shiftcoins Core a
...
- On macOS, Shiftcoins Core now opts out of application CPU throttling
