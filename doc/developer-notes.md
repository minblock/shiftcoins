...
- [Development guidelines](#development-guidelines)
    - [General Shiftcoins Core](#general-shiftcoins-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Shiftcoins Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Run with the `-testnet` option to run with "play shiftcoinss" on the test network, if you
...
Shiftcoins Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/shiftcoins-project/shiftcoins/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_shiftcoins
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_shiftcoins --log_level=test_suite
$ valgrind -v --leak-check=full src/shiftcoinsd -printtoconsole
```
...
# A coverage report will now be accessible at `./test_shiftcoins.coverage/index.html`.
```
...
To profile a running shiftcoinsd process for 60 seconds, you could use an
invocation of `perf record` like this:

```sh
$ perf record \
    -g --call-graph dwarf --per-thread -F 140 \
    -p `pgrep shiftcoinsd` -- sleep 60
```
...
Shiftcoins Core can be compiled with various "sanitizers" enabled, which add
...
However, in open source software such as Shiftcoins Core, where everyone uses
...
pay attention to for reviewers of Shiftcoins Core code.

General Shiftcoins Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: Shiftcoins Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
Some of these are maintained by active developers of Shiftcoins Core, in which case changes should probably go
directly upstream without being PRed directly against the project.  They will be merged back in the next
subtree merge.

Others are external projects without a tight relationship with our project.  Changes to these should also
be sent upstream but bugfixes may also be prudent to PR against Shiftcoins Core so that they can be integrated
...
file descriptors it will cause problems with Shiftcoins's `select()` loop, because
...
would be to revert the upstream fix before applying the updates to Shiftcoins's
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `shiftcoins-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `shiftcoins-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of Shiftcoins Core, the wallet was always
