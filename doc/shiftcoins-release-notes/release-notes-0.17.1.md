Shiftcoins Core version 0.17.1 is now available from:

  <https://download.shiftcoins.org/shiftcoins-0.17.1/>
...
When shiftcoins is configured with the `-deprecatedrpc=accounts` setting, specifying
...
- `-includeconf=<file>` can be used to include additional configuration files.
  Only works inside the `shiftcoins.conf` file, not inside included files or from
  ...
  as shiftcoins.conf will still include `relative.conf`.
...
Previously, wallets could only be loaded or created at startup, by specifying `-wallet` parameters on the command line or in the shiftcoins.conf file. It is now possible to load, create and unload wallets dynamically at runtime:
...
The default minimum transaction fee `-mintxfee` has been lowered to 0.0001 STC/kB after relaxing the minimum relay and dust relay fee rates in prior releases. 
...
prefixing the option with the network, such as:
    ...
    main.uacomment=shiftcoins
    test.uacomment=shiftcoins-testnet
...
be used in V0.17 by starting shiftcoinsd with the '-deprecatedrpc=accounts'
...
BIP 174 Partially Signed Shiftcoins Transactions support
-----------------------------------------------------

[BIP 174 PSBT](https://github.com/bitcoin/bips/blob/master/bip-0174.mediawiki) is an interchange format for Shiftcoins transactions that are not fully signed
yet, together with relevant metadata to help entities work towards signing it.
It is intended to simplify workflows where multiple parties need to cooperate to
produce a transaction. Examples include hardware wallets, multisig setups, and
[CoinJoin](https://bitcointalk.org/?topic=279249) transactions.

For backend RPC convenience, the Shiftcoins devs have supported to keep the acronym `PSBT`
instead of `PSLT` to make crosschain application support easier.

### Overall workflow

Overall, the construction of a fully signed Shiftcoins transaction goes through the
following steps:
...
- An **Extractor** produces a valid Shiftcoins transaction (in network format)
...
- New `fees` field introduced in `getrawmempool`, `getmempoolancestors`, `getmempooldescendants` and
   ...
   and `descendant` denominated in STC. This new field deprecates previous fee fields, such as
   `fee`, `modifiedfee`, `ancestorfee` and `descendantfee`.
- The new RPC `getzmqnotifications` returns information about active ZMQ
  notifications.
- When shiftcoins is not started with any `-wallet=<path>` options, the name of
  the default wallet returned by `getwalletinfo` and `listwallets` RPCs is
  now the empty string `""` instead of `"wallet.dat"`. If shiftcoins is started
...
- `signrawtransaction` is deprecated and will be fully removed in v0.18. To use
  `signrawtransaction` in v0.17, restart shiftcoinsd with
...
- When running shiftcoinsd with `-debug` but without `-daemon`, logging to stdout
...
meaning the `-txindex` flag can be toggled without a full reindex. If shiftcoinsd
