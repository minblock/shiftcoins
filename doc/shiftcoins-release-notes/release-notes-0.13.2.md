Shiftcoins Core version 0.13.2 is now available from:

  <https://download.shiftcoins.org/shiftcoins-0.13.2.1/>
...
Please report bugs using the issue tracker at github:
  ...
  <https://github.com/shiftcoins-project/shiftcoins/issues>
...
updates will be released anymore. Without security updates, using a shiftcoins
...
ECDSA signatures inside Shiftcoins transactions now use validation using
...
Previous versions of Shiftcoins Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Shiftcoins Core was to raise the default minimum
relay fee.

Shiftcoins Core 0.13.2 will have a strict maximum size on the mempool. The
...
Shiftcoins Core 0.13.2 also introduces new default policy limits on the length and
...
This allows running shiftcoinsd without having to do any manual configuration.
...
Shiftcoins Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Shiftcoins Core will relay transactions with
...
entirely in Shiftcoins Core 0.13, and it is at this time undecided whether the
...
Shiftcoins Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
Shiftcoins Core automatically creates a hidden service to listen on, without
manual configuration. Shiftcoins Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if Shiftcoins Core is listening, and
...
Shiftcoinsd can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Shiftcoins
Core to use floating fees. By default, Shiftcoins Core will use floating
fees.
...
(default: `0.0002` STC/kB).

At all times, Shiftcoins Core will cap fees at `-maxtxfee=<x>` (default:
0.10) STC.
Furthermore, Shiftcoins Core will never create transactions paying less than
...
`shiftcoins.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
caching. A sample config for apache2 could look like:
    ...
    <Location /shiftcoinsrpc>
        ...
        # optional bypass shiftcoinsd rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< shiftcoinsrpc:<password>
    </Location>
...
Restarting shiftcoinsd will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in shiftcoins.conf
...
shiftcoins-cli: arguments privacy
------------------------------
...
For example:
    ...
    $ src/shiftcoins-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The Shiftcoins Core code base has
...
- `shiftcoins-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `shiftcoins-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the Shiftcoins scripting system that in combination with
...
Shiftcoins transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of Shiftcoins Core.
...
The two phases are distinct in the Shiftcoins-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Shiftcoins wallet
  software more difficult and which seriously complicates the design of smart
  contracts for Shiftcoins.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each Shiftcoins block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of shiftcoinss they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of shiftcoinss being spent is exactly
...
- **Increased security for multisig:** Shiftcoins addresses (both P2PKH addresses
  ...
  believed to be provided by Shiftcoins's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of Shiftcoins's current ledger (called the UTXO set),
  ...
  note that Shiftcoins Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  Shiftcoins users to individually opt-in to almost any change in the Shiftcoins
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and Shiftcoins Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the Shiftcoins
client to Shiftcoins Core 0.13.2 to signal for segwit support. When a
...
Since Shiftcoins Core 0.10.0, nodes have defaulted to only relaying and
...
- `shiftcoins-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `shiftcoins-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
