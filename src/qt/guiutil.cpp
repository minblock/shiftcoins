...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Shiftcoins address (e.g. %1)").arg(
...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(!uri.isValid() || uri.scheme() != QString("shiftcoins"))
        return false;
...
{
    QString ret = QString("shiftcoins:%1").arg(info.address);
    int paramCount = 0;
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Shiftcoins.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Shiftcoins (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("Shiftcoins (%s).lnk", chain);
}
...
fs::path static GetAutostartFilePath()
{
    std::string chain = gArgs.GetChainName();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "shiftcoins.desktop";
    return GetAutostartDir() / strprintf("shiftcoins-%s.lnk", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Shiftcoins\n";
        else
            optionFile << strprintf("Name=Shiftcoins (%s)\n", chain);
