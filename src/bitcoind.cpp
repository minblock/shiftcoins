...
static bool AppInit(int argc, char* argv[])
{
    ...
    if (HelpRequested(gArgs) || gArgs.IsArgSet("-version")) {
        ...
        {
            strUsage += "\nUsage:  shiftcoinsd [options]                     Start " PACKAGE_NAME " Daemon\n";
    ...
    try
    {
        ...
        for (int i = 1; i < argc; i++) {
            if (!IsSwitchChar(argv[i][0])) {
                tfm::format(std::cerr, "Error: Command line contains unexpected token '%s', see shiftcoinsd -h for a list of options.\n", argv[i]);
                return false;
            }
        }
...
#endif
            tfm::format(std::cout, "Shiftcoins server starting\n");
