    ...
    if (request.fHelp || request.params.size() > 2)
        throw std::runtime_error(
            RPCHelpMan{"getnewaddress",
                "\nReturns a new Shiftcoins address for receiving payments.\n"
                ...
                RPCResult{
            "\"address\"    (string) The new shiftcoins address\n"
                },
                RPCExamples{
    ...
    if (request.fHelp || request.params.size() > 1)
        throw std::runtime_error(
            RPCHelpMan{"getrawchangeaddress",
                "\nReturns a new Shiftcoins address, for receiving change.\n"
...
static UniValue setlabel(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 2)
        throw std::runtime_error(
            RPCHelpMan{"setlabel",
                ...
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The shiftcoins address to be associated with a label."},
    ...
    if (!IsValidDestination(dest)) {
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Shiftcoins address");
    }
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 8)
        throw std::runtime_error(
            RPCHelpMan{"sendtoaddress",
                ...
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The shiftcoins address to send to."},
            ...
            "                             The recipient will receive less shiftcoinss than you enter in the amount field."},
...
static UniValue listaddressgroupings(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "  [\n"
            "    [\n"
            "      \"address\",            (string) The shiftcoins address\n"
        ...
        throw std::runtime_error(
            RPCHelpMan{"signmessage",
                "\nSign a message with the private key of an address" +
                    HelpRequiringPassphrase(pwallet) + "\n",
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The shiftcoins address to use for the private key."},
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            RPCHelpMan{"getreceivedbyaddress",
                ...
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The shiftcoins address for transactions."},
...
static UniValue sendmany(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 8)
        throw std::runtime_error(
            RPCHelpMan{"sendmany",
                    ...
                    HelpRequiringPassphrase(pwallet) + "\n",
                {
                    ...
                    {"amounts", RPCArg::Type::OBJ, RPCArg::Optional::NO, "A json object with addresses and amounts",
                        {
                            {"address", RPCArg::Type::AMOUNT, RPCArg::Optional::NO, "The shiftcoins address is the key, the numeric amount (can be string) in " + CURRENCY_UNIT + " is the value"},
                        },
                    },
            ...
            "                           Those recipients will receive less shiftcoinss than you enter in their corresponding amount field.\n"
    ...
    for (const std::string& name_ : keys) {
        ...
        if (!IsValidDestination(dest)) {
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Shiftcoins address: ") + name_);
        }
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 4) {
        std::string msg =
            RPCHelpMan{"addmultisigaddress",
                ...
                "Each key is a Shiftcoins address or hex-encoded public key.\n"
                "This functionality is only intended for use with non-watchonly addresses.\n"
                "See `importaddress` for watchonly p2sh address support.\n"
                "If 'label' is specified, assign address to that label.\n",
                {
                    {"nrequired", RPCArg::Type::NUM, RPCArg::Optional::NO, "The number of required signatures out of the n keys or addresses."},
                    {"keys", RPCArg::Type::ARR, RPCArg::Optional::NO, "A json array of shiftcoins addresses or hex-encoded public keys",
                        {
                            {"key", RPCArg::Type::STR, RPCArg::Optional::OMITTED, "shiftcoins address or hex-encoded public key"},
                        },
                        },
...
UniValue listtransactions(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 4)
        throw std::runtime_error(
            ...
            "[\n"
            "  {\n"
            "    \"address\":\"address\",    (string) The shiftcoins address of the transaction.\n"
...
static UniValue gettransaction(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "  \"details\" : [\n"
            "    {\n"
            "      \"address\" : \"address\",          (string) The shiftcoins address involved in the transaction\n"
            "      \"category\" :                      (string) The transaction category.\n"
            "                   \"send\"                  Transactions sent.\n"
            "                   \"receive\"               Non-coinbase transactions received.\n"
            "                   \"generate\"              Coinbase transactions received with more than 100 confirmations.\n"
    ...
    if (request.fHelp || request.params.size() != 2) {
        throw std::runtime_error(
            RPCHelpMan{"walletpassphrase",
                ...
                "This is needed prior to performing transactions related to private keys such as sending shiftcoinss\n"
            "\nNote:\n"
...
static UniValue encryptwallet(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1) {
        throw std::runtime_error(
            ...
            "\nNow set the passphrase to use the wallet, such as for signing or sending shiftcoins\n"
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            RPCHelpMan{"lockunspent",
                ...
                "A locked transaction output will not be chosen by automatic coin selection, when spending shiftcoinss.\n"
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            RPCHelpMan{"loadwallet",
                ...
                "\nNote that all wallet command-line options used when starting shiftcoinsd will be"
...
static UniValue listunspent(const JSONRPCRequest& request)
{
        ...
        throw std::runtime_error(
            RPCHelpMan{"listunspent",
                ...
                "Optionally filter to only include txouts paid to specified addresses.\n",
                {
                    ...
                    {"addresses", RPCArg::Type::ARR, /* default */ "empty array", "A json array of shiftcoins addresses to filter",
                        {
                            {"address", RPCArg::Type::STR, RPCArg::Optional::OMITTED, "shiftcoins address"},
                        },
                    },
            ...
            "    \"vout\" : n,               (numeric) the vout value\n"
            "    \"address\" : \"address\",    (string) the shiftcoins address\n"
    ...
    if (!request.params[2].isNull()) {
        ...
        for (unsigned int idx = 0; idx < inputs.size(); idx++) {
            ...
            if (!IsValidDestination(dest)) {
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Shiftcoins address: ") + input.get_str());
            }
    ...
    if (!options.isNull()) {
      if (options.type() == UniValue::VBOOL) {
        ...
        if (options.exists("changeAddress")) {
            ...
            if (!IsValidDestination(dest)) {
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "changeAddress must be a valid shiftcoins address");
            }
    ...
    const RPCHelpMan help{"fundrawtransaction",
                ...
                "Only pay-to-pubkey, multisig, and P2SH versions thereof are currently supported for watch-only\n",
                {
                    ...
                    {"options", RPCArg::Type::OBJ, RPCArg::Optional::OMITTED_NAMED_ARG, "for backward compatibility: passing in a true instead of an object will result in {\"includeWatching\":true}",
                        {
                            {"changeAddress", RPCArg::Type::STR, /* default */ "pool address", "The shiftcoins address to receive the change"},
...
UniValue generate(const JSONRPCRequest& request)
{
    ...
    if (!IsDeprecatedRPCEnabled("generate")) {
        throw JSONRPCError(RPC_METHOD_DEPRECATED, "The wallet generate rpc method is deprecated and will be fully removed in v0.19. "
            "To use generate in v0.18, restart shiftcoinsd with -deprecatedrpc=generate.\n"
    ...
    if (request.fHelp || request.params.size() != 1) {
        throw std::runtime_error(
            RPCHelpMan{"getaddressinfo",
                "\nReturn information about the given shiftcoins address. Some information requires the address\n"
                "to be in the wallet.\n",
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The shiftcoins address to get the information of."},
                },
                RPCResult{
            "{\n"
            "  \"address\" : \"address\",        (string) The shiftcoins address validated\n"
                ...
                "Implements the Creator and Updater roles.\n",
                {
                    ...
                    {"outputs", RPCArg::Type::ARR, RPCArg::Optional::NO, "a json array with outputs (key-value pairs), where none of the keys are duplicated.\n"
                            ...
                            "                             accepted as second parameter.",
                        {
                            {"", RPCArg::Type::OBJ, RPCArg::Optional::OMITTED, "",
                                {
                                    {"address", RPCArg::Type::AMOUNT, RPCArg::Optional::NO, "A key-value pair. The key (string) is the shiftcoins address, the value (float or string) is the amount in " + CURRENCY_UNIT + ""},
                                },
                                },
                            {"", RPCArg::Type::OBJ, RPCArg::Optional::OMITTED, "",
                                {
                    ...
                    {"options", RPCArg::Type::OBJ, RPCArg::Optional::OMITTED_NAMED_ARG, "",
                        {
                            {"changeAddress", RPCArg::Type::STR_HEX, /* default */ "pool address", "The shiftcoins address to receive the change"},
                            {"changePosition", RPCArg::Type::NUM, /* default */ "random", "The index of the change output"},
                            {"change_type", RPCArg::Type::STR, /* default */ "set by -changetype", "The output type to use. Only valid if changeAddress is not specified. Options are \"legacy\", \"p2sh-segwit\", and \"bech32\"."},
                            {"includeWatching", RPCArg::Type::BOOL, /* default */ "false", "Also select inputs which are watch only"},
                            {"lockUnspents", RPCArg::Type::BOOL, /* default */ "false", "Lock selected unspent outputs"},
