...
UniValue importaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 4)
        throw std::runtime_error(
            ...
            "as change, and not show up in many RPCs.\n",
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The Shiftcoins address (or hex-encoded script)"},
...

    {
        ...
        if (IsValidDestination(dest)) {
            ...
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Shiftcoins address or script");
        }
    }
    if (fRescan)
    {
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            RPCHelpMan{"dumpprivkey",
                ...
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The shiftcoins address for the private key"},
                },
                RPCResult{
...
UniValue dumpwallet(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            RPCHelpMan{"dumpwallet",
                ...
                {
                    {"filename", RPCArg::Type::STR, RPCArg::Optional::NO, "The filename with path (either absolute or relative to shiftcoinsd)"},
                },
                RPCResult{
            "{                           (json object)\n"
    ...
    // produce output
    file << strprintf("# Wallet dump created by Shiftcoins %s\n", CLIENT_BUILD);
                    ...
                    result.pushKV(
                        ...
                        JSONRPCError(
                            ...
                            strprintf("Rescan failed for key with creation timestamp %d. There was an error reading a "
                                      ...
                                      "caused by pruning or data corruption (see shiftcoinsd log for details) and could "
