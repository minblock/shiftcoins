...
const char * const BITCOIN_CONF_FILENAME = "shiftcoins.conf";
...
#else
    const char* pszModule = "shiftcoins";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Shiftcoins";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Shiftcoins";
#else
    // Unix
    return pathRet / ".shiftcoins";
#endif
#endif
}
