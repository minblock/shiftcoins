...
static bool WalletAppInit(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs)) {
        std::string usage = strprintf("%s shiftcoins-wallet version", PACKAGE_NAME) + " " + FormatFullVersion() + "\n\n" +
                                      "wallet-tool is an offline tool for creating and interacting with Shiftcoins Core wallet files.\n" +
                                      "By default wallet-tool will act on wallets in the default mainnet wallet directory in the datadir.\n" +
                                      "To change the target wallet, use the -datadir, -wallet and -testnet/-regtest arguments.\n\n" +
                                      "Usage:\n" +
                                     "  shiftcoins-wallet [options] <command>\n\n" +
...
int main(int argc, char* argv[])
{
    ...
    if (method.empty()) {
        tfm::format(std::cerr, "No method provided. Run `shiftcoins-wallet -help` for valid methods.\n");
        return EXIT_FAILURE;
    }
