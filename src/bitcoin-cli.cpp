...
static void SetupCliArgs()
{
    ...
    gArgs.AddArg("-rpcwallet=<walletname>", "Send RPC for non-default wallet on RPC server (needs to exactly match corresponding -wallet option passed to shiftcoinsd). This changes the RPC endpoint used, e.g. http://127.0.0.1:9332/wallet/<walletname>", false, OptionsCategory::OPTIONS);
...
static int AppInitRPC(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.IsArgSet("-version")) {
        ...
        if (!gArgs.IsArgSet("-version")) {
            strUsage += "\n"
                "Usage:  shiftcoins-cli [options] <command> [params]  Send command to " PACKAGE_NAME "\n"
                "or:     shiftcoins-cli [options] -named <command> [name=value]...  Send command to " PACKAGE_NAME " (with named arguments)\n"
                "or:     shiftcoins-cli [options] help                List commands\n"
                "or:     shiftcoins-cli [options] help <command>      Get help for a command\n";
...
static UniValue CallRPC(BaseRequestHandler *rh, const std::string& strMethod, const std::vector<std::string>& args)
{
    ...
    if (response.status == 0) {
        ...
        throw CConnectionFailed(strprintf("Could not connect to the server %s:%d%s\n\nMake sure the shiftcoinsd server is running and that you are connecting to the correct RPC port.", host, port, responseErrorMessage));
                ...
                if (!error.isNull()) {
                    ...
                    if (error.isObject())
                    {
                        ...
                        if (errCode.isNum() && errCode.get_int() == RPC_WALLET_NOT_SPECIFIED) {
                            strPrint += "\nTry adding \"-rpcwallet=<filename>\" option to shiftcoins-cli command line.";
                        }
                    }
                } else {
                    // Result
