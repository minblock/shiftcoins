...
def run_tests(*, test_list, src_dir, build_dir, tmpdir, jobs=1, enable_coverage=False, args=None, combined_logs_len=0, failfast=False, runs_ci):
    ...
    try:
        if subprocess.check_output(["pidof", "shiftcoinsd"]) is not None:
            print("%sWARNING!%s There is already a shiftcoinsd process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
...
class RPCCoverage():
    ...
    commands per `shiftcoins-cli help` (`rpc_interface.txt`).
